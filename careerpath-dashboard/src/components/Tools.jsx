  ...
  const tools = [
    {
      ...
      onClick: null // Logic handled by other buttons or top nav
    },
    {
    ...
    <section className="py-24 px-8 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-7xl">
        {tools.map((tool, index) => (
          <div 
            key={index} 
            className={`glass-card rounded-2xl p-8 flex flex-col transition-all duration-300 ${tool.onClick ? 'cursor-pointer hover:border-primary/50 hover:bg-primary/5' : ''}`}
