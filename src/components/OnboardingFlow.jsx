...
const STEPS = [
  {
    ...
    subtitle: "Tell us your class, stream, and goals",
    icon: "🎯",
  },
  {
    id: 2,
    title: "Strengths & Gaps",
    subtitle: "Where you shine and where you struggle",
    icon: "💡",
  },
  {
    id: 3,
    title: "Study Habits",
    subtitle: "How and when you study best",
    icon: "📚",
  },
  {
    id: 4,
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        ...
        <button
          ...
          onClick={() => {
            setShowOther((p) => !p);
            if (!showOther) onToggle("__other__");
            else {
              onToggle("__other__");
              onOtherChange("");
            }
...
const LoadingScreen = ({ activeStep }) => (
  <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-8">
    ...
    <h2 className="text-2xl font-bold text-white mb-2 text-center">
      ShikshaSetu AI is Thinking...
    </h2>
    <p className="text-gray-400 text-sm mb-10 text-center">
      Gemini is deeply analyzing your academic profile
    </p>
    <div className="w-full max-w-xs space-y-3">
      {LOADING_STEPS.map((s, i) => {
        ...
        return (
          <div
            key={i}
            className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-500 ${active ? "bg-indigo-500/10 border-indigo-500/30 text-white" : done ? "bg-white/[0.02] border-white/5 text-gray-500" : "bg-transparent border-transparent text-gray-700"}`}
          >
            <div
              className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 border transition-all ${done ? "bg-green-500 border-green-500" : active ? "border-indigo-400 animate-pulse" : "border-white/10"}`}
            >
              {done && <CheckIcon />}
              {active && (
    ...
    <p className="text-gray-600 text-[11px] mt-8">
...
const OnboardingFlow = ({ onComplete, onClose, userId }) => {
  ...
  const [_dir, setDir] = useState(1); // 1 = forward, -1 = backward
  ...
  const [data, setData] = useState({
    ...
    subjectEntries: [
      { subject: "", score: "", correctQ: "", incorrectQ: "", accuracy: "" },
    ],
    weakTopics: [],
    otherWeakTopic: "",
    mistakeType: "",
  ...
  const removeEntry = (idx) =>
    setData((p) => ({
      ...p,
      subjectEntries: p.subjectEntries.filter((_, i) => i !== idx),
    }));
  ...
  const canNext = () => {
    ...
    if (step === 2)
      return data.studyHours && data.studyTime && data.practiceStyle;
    if (step === 3)
      return data.subjectEntries[0].subject && data.subjectEntries[0].score;
    return false;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const enrichedData = {
        ...
        practiceStyleAll:
          data.practiceStyle === "Other (specify)" && data.otherStudyStyle
            ? data.otherStudyStyle
            : data.practiceStyle,
        mistakeTypeAll:
          data.mistakeType === "Other (specify)" && data.otherMistakeType
            ? data.otherMistakeType
            : data.mistakeType,
      ...
      if (!res.ok) {
        ...
        if (DEV_MODE) {
          console.warn(
            "DEV_MODE active: Using fallback mock AI profile to bypass API quotas.",
          );
          clearInterval(iv);
          ...
          const mockOutput = {
            ...
            recommendedFocus: [
              {
                subject: "Computer Science Development",
                priority: "high",
                reason: "Build core systems.",
                actionPlan: ["Review docs", "Build projects"],
              },
            ],
            careerSuggestions: [
              {
                career: "Software Engineer (DEV FALLBACK)",
                matchScore: 99,
                reason: "Matches all developmental attributes perfectly.",
              },
            ],
            insights: {
              strengthSummary: "Great logic.",
      ...
      if (uid !== "anonymous") {
        ...
        setDoc(ref, {
          ...
          .catch((e) =>
            console.warn("[Firestore] Background save failed:", e.message),
          );
      }
    } catch (err) {
      clearInterval(iv);
      setError(
        err.message || "Failed to reach AI. Make sure the server is running.",
      );
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-gray-200 flex font-sans ">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-4 rounded-xl text-left transition-all duration-300 ${
                  active
                    ? "bg-indigo-500/10 border border-indigo-500/30 shadow-inner text-white"
                    : done
                      ? "bg-[#1f2937]/30 border border-transparent text-gray-400 hover:bg-[#1f2937]/50"
              ...
              >
                ...
                <div className="min-w-0">
                  <p
                    className={`text-sm font-bold truncate ${active ? "text-indigo-100" : ""}`}
                  >
                    {s.title}
                  </p>
                </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 w-full">
          <div
            className="max-w-[900px] mx-auto p-6 md:p-12"
            key={step}
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      <p className="text-xs text-gray-500 mb-4 font-medium">
                        Core subjects will dynamically align to your selection.
                      </p>
                      ...
                      {data.targetGoal && (
                        <div className="mt-6 bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 text-sm text-indigo-300 flex items-start gap-3">
                          <SparkleIcon />
                          <p>
                            Subjects actively loaded for{" "}
                            <strong>{data.targetGoal}</strong>:
                            <span className="block mt-1 text-gray-300">
                              {adaptiveSubjects.join(", ")}
                            </span>
                          </p>
                        </div>
                      )}
                ...
                <div className="space-y-10">
                  <div>
                    ...
                    <div className="mb-8">
                      <Label>
                        Strong Subjects{" "}
                        <span className="text-indigo-400 normal-case font-normal text-xs ml-1">
                          (Pick all that apply)
                        </span>
                      </Label>
                      <ChipGroup
                    ...
                    <div className="mb-4">
                      <Label>
                        Weak Subjects{" "}
                        <span className="text-red-400 normal-case font-normal text-xs ml-1">
                          (Pick all that apply)
                        </span>
                      </Label>
                      <ChipGroup
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          set("biggestChallenge", e.target.value)
                        }
                        rows={3}
                        placeholder="e.g. I struggle with long numerical problems under time pressure..."
                  ...
                  <div>
                    ...
                    <div className="mb-8">
                      ...
                      <RadioGroup
                        ...
                        onChange={(v) => set("practiceStyle", v)}
                      />
                      {data.practiceStyle === "Other (specify)" && (
                        <input
                          value={data.otherStudyStyle}
                          onChange={(e) =>
                            set("otherStudyStyle", e.target.value)
                          }
                        ...
                        {data.uploadedFileName ? (
                          <div className="flex items-center justify-center gap-3 text-green-400">
                            ...
                            <button
                              ...
                              className="ml-3 text-gray-500 hover:text-red-400 transition-colors p-2"
                            >
                              <TrashIcon />
                            </button>
                          </div>
                          ...
                          <>
                            <div className="flex justify-center mb-4 text-gray-500 group-hover:text-indigo-400 transition-colors">
                              <UploadIcon />
                            </div>
                            <p className="text-base font-semibold text-gray-300 mb-1">
                              Upload your Latest Test Result
                            </p>
                            <p className="text-sm text-gray-500">
                              AI directly parses subjects, marks, and mistakes
                            </p>
                          </>
                        )}
                      </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="space-y-4">
                        {data.subjectEntries.map((entry, idx) => (
                          <div
                            key={idx}
                            className="bg-[#1f2937]/30 border border-[#1f2937] rounded-[16px] p-6 relative"
                          >
                            {data.subjectEntries.length > 1 && (
                              <button
                                type="button"
                                onClick={() => removeEntry(idx)}
                                className="absolute top-4 right-4 text-gray-500 hover:text-red-400 transition-colors bg-[#111827] p-1.5 rounded-lg border border-[#374151]"
                              >
                                <TrashIcon />
                              </button>
                            )}
                              ...
                              {[
                                ...
                                <div key={fld}>
                                  ...
                                  <input
                                    ...
                                    value={entry[fld]}
                                    onChange={(e) =>
                                      updateEntry(idx, fld, e.target.value)
                                    }
                                    placeholder={ph}
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      <RadioGroup
                        ...
                        onChange={(v) => set("mistakeType", v)}
                      />
                      {data.mistakeType === "Other (specify)" && (
                        <input
                          value={data.otherMistakeType}
                          onChange={(e) =>
                            set("otherMistakeType", e.target.value)
                          }
            ...
            {step < 3 ? (
              <button
                ...
                className={`flex items-center gap-2 font-bold py-3 px-8 rounded-xl text-sm transition-all duration-300 ${
                  !canNext()
                    ? "bg-[#1f2937]/30 text-gray-600 border border-[#1f2937]/50 cursor-not-allowed"
                    : "bg-indigo-600 hover:bg-indigo-500 text-white shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_25px_rgba(79,70,229,0.5)] border border-indigo-500 active:scale-95"
                }`}
              >
                Continue <ArrowRight />
              </button>
              ...
              <button
                ...
                className={`flex items-center gap-2 font-bold py-3 px-8 rounded-xl text-sm transition-all duration-300 ${
                  !canNext()
                    ? "bg-[#1f2937]/30 text-gray-600 border border-[#1f2937]/50 cursor-not-allowed"
                    : "bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_25px_rgba(79,70,229,0.5)] border border-indigo-500 active:scale-95"
                }`}
              >
